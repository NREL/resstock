...
class IRAAnalysis:
    def __init__(
        ...
        if not self.tribal_map_file.exists():
            raise ValueError(
                f"Tribal mapping file does not exist: {self.tribal_map_file} "
    ...
    def _remap_result_to_tribal_nations(self, DF2, groupby_cols):
        ...
        tribal_map = (
            pd.read_csv(self.tribal_map_file)
            .drop(columns=["PUMA", "State", "sampling_probability"])
            .set_index(["PUMA_nhgis"])
        )
        ...
        DF2[metric_cols] = DF2[metric_cols].multiply(
            DF2["applicable_household_count"], axis=0
        )
        DF3 = DF2.groupby(groupby_cols)[count_cols].sum()
        DF3[metric_cols] = (
            DF2.groupby(groupby_cols)[metric_cols]
            .sum()
            .divide(
    ...
    def load_tribal_hc_breakdown_file(self, hc_cols):
        ...
        df_weight["PUMA"] = df_weight["PUMA"].map(
    ...
    def calculate_new_sample_weight_tribal_as_map(self):
        ...
        df_cols = [
            "in." + col.replace(" Tribal", "").replace(" ", "_").lower()
            for col in hc_cols
        ]  # cols used in EUSS

        weight_sum = df["weight"].sum()
        df_weight = self.load_tribal_hc_breakdown_file(hc_cols)
        df_weight = df_weight.rename(columns=dict(zip(hc_cols, df_cols))).set_index(
            df_cols
        )
        df_weight = df_weight.join(
            df.groupby(df_cols)["weight"].count().rename("count"), how="right"
        )
        ...
        sa_puma_aia_na = tribal_map[tribal_map["PUMA_nhgis"].isin(puma_aia_na)][
            "sampling_probability"
        ].sum()
        sa_puma_aia = tribal_map[tribal_map["percent_hu"] > 0][
            "sampling_probability"
        ].sum()

        print("WARING : ")
        print(
            "Redistributing weight based on Tribal Housing Characteristics will nullify / remove "
            f"{len(df_na)} / {len(df)} bldgs ({(len(df_na)/len(df)*100):02f} %)"
        )
        print(
            f"This at least partially affects {len(puma_aia_na)} PUMA regions that contain AIAs "
        ...
        df_weight["new_weight"] = (
            df_weight["sampling_probability"] / df_weight["sampling_probability"].sum()
        ).divide(df_weight["count"], axis=0) * weight_sum
        # scale back up to baseline
        df_weight = df_weight.join(
            df.set_index(df_cols)[["bldg_id", "weight"]], how="right"
        ).reset_index()

        # QC - total of redistributed weight is the same as before
        assert round(df_weight["new_weight"].sum(), 0) == round(
            weight_sum, 0
        ), f"New weight and old weight are not approximately equal:\n{df_weight[['new_weight', 'weight']].sum()}"

        df_weight.to_csv(
            self.data_dir / "EUSS_weight_redistribution_for_tribal_nations.csv"
        )
