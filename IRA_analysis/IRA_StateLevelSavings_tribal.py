...
class IRAAnalysis():
    def __init__(self, euss_dir, groupby_cols, coarsening_map, emission_type, output_dir=None):
        ...
        print(f"""
            ...
            """)
        print("=========================================================================")
        print(f"Energy/savings are grouped by: {groupby_cols}\nwith coarsening_map: {coarsening_map}")
    ...
    def get_groupby_cols_with_coarsening(groupby_cols, coarsening_map=None):
        if coarsening_map is None:
            ...
            assert isinstance(coarsening_map, dict), "only support single coarsening_map currently, coarsening_map: None or dict."
            return [coarsening_map[col] if col in coarsening_map else col for col in groupby_cols]
    ...
    def get_emission_cols(self, emission_type="lrmer_low_re_cost_25_2025_start", output="all"):
        cols = self.get_emission_savings_cols(emission_type=emission_type, output=output)
        if isinstance(cols, str):
    ...
    def validate_value_column(value_col):
        if value_col.endswith(".kwh.savings") or value_col.endswith(".kwh"):
            ...
            raise ValueError(f"value_col={value_col} cannot be used with calculate_mean_values")
    ...
    def calculate_mean_values(self, df, groupby_cols: list, value_col: str, as_percentage=False):
    ...
    @staticmethod
    def calculate_confidence_intervals(self, df, groupby_cols: list, value_col: str, confidence="95%CI"):
    ...
    def load_results(self, pkgs: list):
        ...
        if isinstance(pkgs, list):
            if 0 in pkgs:
                raise ValueError("cannot combined baseline results (pkg0) with upgrade results.")
            DF = []
            for pkg in pkgs:
    ...
    def _get_mean_dataframe_base(self, df, groupby_cols: list, energy_savings_cols: list, total_emission_col: str, as_percentage=False):
        ...
        for energy_col in energy_savings_cols:
            DF.append(self.calculate_mean_values(df, groupby_cols, energy_col, as_percentage=as_percentage))
            print(f" - added {energy_col}")
        DF.append(self.calculate_mean_values(df, groupby_cols, total_emission_col, as_percentage=as_percentage))
    ...
    def _get_mean_dataframe_with_coarsening(self, df, groupby_cols: list, energy_savings_cols: list, total_emission_col: str, as_percentage=False, coarsening_map=None):

        if coarsening_map is None:
            return self._get_mean_dataframe_base(df, groupby_cols, energy_savings_cols, total_emission_col, as_percentage=as_percentage)

        # get mean savings for coarsened groupby_cols
        groupby_cols_coarsened = self.get_groupby_cols_with_coarsening(groupby_cols, coarsening_map=coarsening_map)
        DF = self._get_mean_dataframe_base(
            df, groupby_cols_coarsened, energy_savings_cols, total_emission_col, as_percentage=as_percentage
            )
        ...
        df_map = df_map.div(df_map.groupby(groupby_cols).sum()) # normalize to give breakdown of mapped_col by orig_col
        df_map = df_map.rename("fraction").to_frame().reset_index().set_index(groupby_cols_coarsened)

        DF2 = df_map.join(DF, how="left").groupby(groupby_cols).apply(
            lambda x: x[DF.columns].multiply(x["fraction"], axis=0).sum()
    ...
    def _replace_savings_cols_as_percentage(self, df, energy_saving_cols, total_emission_saving_col):
        ...
        total_emission_col = self.get_emission_cols(emission_type=self.emission_type, output="total_fuel")
        total_emission_saving_col_baseline = self.get_emission_savings_cols(emission_type=self.emission_type, output="total_fuel")

        assert len(energy_saving_cols) == len(energy_cols) == len(energy_saving_cols_baseline), \
        ...
        for saving_col, total_col, saving_col_baseline in zip(energy_saving_cols, energy_cols, energy_saving_cols_baseline):
            baseline = (dfi[[total_col, saving_col_baseline]].sum(axis=1)) # upgraded_baseline + saving = pre-upgrade baseline
            dfi[saving_col] /= baseline
        baseline_emission = (dfi[[total_emission_col, total_emission_saving_col_baseline]].sum(axis=1)) # <-- baseline CO2
    ...
    def validate_percent_savings(self, DF):
        ...
        df_over_100pct = DF[(DF[fraction_cols]>1) | (DF[fraction_cols]<-1)].dropna(how="all")
        if len(df_over_100pct)>0:
            print(f"{len(df_over_100pct)} / {len(DF)} have percent savings or increase beyond 100%")
    ...
    def _create_new_cols_for_enduse_(self, df, enduse, energy_cols_enduse, energy_cols_total, emission_cols):
        ...
        total_col = self.get_energy_savings_cols(end_use="total", output="total_fuel").replace("total", enduse)
        dfi[total_col] = dfi[energy_cols_enduse].sum(axis=1)
        energy_cols_enduse.append(total_col)

        total_emission_col = self.get_emission_savings_cols(emission_type=self.emission_type, output="total_fuel")\
    ...
    def _get_savings_dataframe_for_an_enduse(self, df, enduse, as_percentage=False, coarsening=True):
        energy_saving_cols_enduse = self.get_energy_savings_cols(end_use=enduse, output="by_fuel")
        energy_saving_cols_total = self.get_energy_savings_cols(end_use="total", output="by_fuel")
        emission_saving_cols = self.get_emission_savings_cols(emission_type=self.emission_type, output="by_fuel")

        dfi, energy_saving_cols_enduse, total_emission_saving_col = self._create_new_cols_for_enduse_(df, enduse, energy_saving_cols_enduse, energy_saving_cols_total, emission_saving_cols)
        
        if as_percentage:
            dfi = self._replace_savings_cols_as_percentage(dfi, energy_saving_cols_enduse, total_emission_saving_col)

        # get mean savings
        coarsening_map = self.coarsening_map if coarsening else None
        DF = self._get_mean_dataframe_with_coarsening(
            dfi, self.groupby_cols, energy_saving_cols_enduse, total_emission_saving_col, 
            as_percentage=as_percentage, coarsening_map=coarsening_map
            )
    ...
    def _get_savings_dataframe(self, df, as_percentage=False, coarsening=True):
        ...
        total_emission_saving_col = self.get_emission_savings_cols(emission_type=self.emission_type, output="total_fuel")
        dfi = df.copy()

        if as_percentage:
            dfi = self._replace_savings_cols_as_percentage(df, energy_saving_cols, total_emission_saving_col)

        # get mean savings
        coarsening_map = self.coarsening_map if coarsening else None
        DF = self._get_mean_dataframe_with_coarsening(
            dfi, self.groupby_cols, energy_saving_cols, total_emission_saving_col, 
            as_percentage=as_percentage, coarsening_map=coarsening_map
            )
    ...
    def get_savings_total(self, pkg, pkg_name, as_percentage=False, coarsening=True, return_df=False):
        ...
        df = df.loc[cond]
        DF = self._get_savings_dataframe(df, as_percentage=as_percentage, coarsening=coarsening)
    ...
    def get_savings_dryer(self, pkg, pkg_name, as_percentage=False, coarsening=True, return_df=False):
        ...
        DF = self._get_savings_dataframe_for_an_enduse(df, enduse, as_percentage=as_percentage, coarsening=coarsening)
    ...
    def _create_new_cols_for_heating_and_cooling_(self, df):
        ...
        """
        energy_cols_hot_water = self.get_energy_savings_cols(end_use="hot_water", output="by_fuel")
        energy_cols_dryer = self.get_energy_savings_cols(end_use="clothes_dryer", output="by_fuel")
        energy_cols_cooking = self.get_energy_savings_cols(end_use="range_oven", output="by_fuel")
        energy_cols_total = self.get_energy_savings_cols(end_use="total", output="by_fuel")
        emission_cols = self.get_emission_savings_cols(emission_type=self.emission_type, output="by_fuel")
        ...
        # combined attributed savings, add as new cols
        total_col = self.get_energy_savings_cols(end_use="total", output="total_fuel").replace("total", enduse_new)
    ...
    def get_savings_heating_and_cooling(self, pkg, pkg_name, as_percentage=False, coarsening=True, return_df=False):
        ...
        dfi, energy_cols_new, total_emission_col = self._create_new_cols_for_heating_and_cooling_(df)
        if as_percentage:
            dfi = self._replace_savings_cols_as_percentage(dfi, energy_cols_new, total_emission_col)

        # get mean savings
        coarsening_map = self.coarsening_map if coarsening else None
        DF = self._get_mean_dataframe_with_coarsening(dfi, self.groupby_cols, energy_cols_new, total_emission_col, as_percentage=as_percentage, coarsening_map=coarsening_map)
    ...
    def get_baseline_consumption(self, pkg, pkg_name, coarsening=True, return_df=False):
        ...
        # get mean savings
        coarsening_map = self.coarsening_map if coarsening else None
        DF = self._get_mean_dataframe_with_coarsening(df, self.groupby_cols, energy_cols, total_emission_col, coarsening_map=coarsening_map)
    ...
    def get_segment_count_in_baseline(self):
        ...
        cond = df["in.vacancy_status"]=="Occupied"
        df = df.loc[cond]
        DF = df.groupby(self.groupby_cols)["weight"].sum().rename("total_household_count").to_frame()

        return DF

    def get_segment_fraction_above_targets(self, pkg, pkg_name, end_use="total", saving_targets=[0.15, 0.20, 0.35], return_df=False):
        ...
        if end_use == "total":
            ...
            dfi, energy_saving_cols, total_emission_saving_col = self._create_new_cols_for_enduse_(df, enduse, energy_saving_cols_enduse, energy_saving_cols_total, emission_saving_cols)
            ...
            dfi, energy_saving_cols, total_emission_saving_col = self._create_new_cols_for_heating_and_cooling_(df)

        else:
            raise ValueError(f"Invalide, end_use={end_use}, only 'total', dryer', 'cooking', and 'heat_cool' supported")

        # get savings as pct
        dfi = self._replace_savings_cols_as_percentage(dfi, energy_saving_cols, total_emission_saving_col)

        # get fractions
        site_saving_pct = self.get_energy_savings_cols(end_use="total", output="total_fuel")
        DF = dfi.groupby(self.groupby_cols).apply(lambda x: self._fraction_stats_(x, saving_targets=saving_targets))
    ...
    def _fraction_stats_(self, df, saving_targets):
        ...
        for goal in saving_targets:
            ...
            fraction = df.loc[df[site_saving_pct]>=goal, site_saving_pct].count() / df[site_saving_pct].count()
            dfo[label] = fraction
...
def main(euss_dir):
    ...
    IRA.get_savings_total(2, "enhanced_enclosure_upgrade", as_percentage=as_percentage, coarsening=coarsening)

    # [3] Heat pump – min eff: all (pkg 3)
    IRA.get_savings_total(3, "heat_pump_min_eff_with_electric_backup", as_percentage=as_percentage, coarsening=coarsening)

    # [4] Heat pump – high eff: all (pkg 4)
    IRA.get_savings_total(4, "heat_pump_high_eff_with_electric_backup", as_percentage=as_percentage, coarsening=coarsening)

    # [5] Heat pump – min eff + existing backup: all (pkg 5)
    IRA.get_savings_total(5, "heat_pump_min_eff_with_existing_backup", as_percentage=as_percentage, coarsening=coarsening)

    # [6] Heat pump – high eff + basic enclosure: Heating & Cooling (pkg 9)
    IRA.get_savings_heating_and_cooling(9, "heat_pump_high_eff_with_basic_enclosure", as_percentage=as_percentage, coarsening=coarsening)

    # [7] Heat pump – high eff + enhanced enclosure: Heating & Cooling (pkg 10)
    IRA.get_savings_heating_and_cooling(10, "heat_pump_high_eff_with_enhanced_enclosure", as_percentage=as_percentage, coarsening=coarsening)

    # [8] Heat pump water heater: all (pkg 6)
    IRA.get_savings_total(6, "heat_pump_water_heater", as_percentage=as_percentage, coarsening=coarsening)

    # [9] Electric dryer: Clothes dryer (pkg 7)
    IRA.get_savings_dryer(7, "electric_clothes_dryer", as_percentage=as_percentage, coarsening=coarsening)

    # [10] Heat pump dryer: Clothes dryer (pkg 8, 9, 10)
    IRA.get_savings_dryer([8,9,10], "heat_pump_clothes_dryer", as_percentage=as_percentage, coarsening=coarsening)

    # [11] Electric cooking: Cooking (pkg 7)
    IRA.get_savings_cooking(7, "electric_cooking", as_percentage=as_percentage, coarsening=coarsening)

    # [12] Induction cooking: Cooking (pkg 8, 9, 10)
    IRA.get_savings_cooking([8,9,10], "induction_cooking", as_percentage=as_percentage, coarsening=coarsening)
...
def main_goal_achievement(euss_dir):
    ...
    # Do calculations
    # [1] Basic enclosure: all (pkg 1)
    IRA.get_segment_fraction_above_targets(1, "basic_enclosure_upgrade", end_use="total")

    # [2] Enhanced enclosure: all (pkg 2)
    IRA.get_segment_fraction_above_targets(2, "enhanced_enclosure_upgrade", end_use="total")

    # [3] Heat pump – min eff: all (pkg 3)
    IRA.get_segment_fraction_above_targets(3, "heat_pump_min_eff_with_electric_backup", end_use="total")

    # [4] Heat pump – high eff: all (pkg 4)
    IRA.get_segment_fraction_above_targets(4, "heat_pump_high_eff_with_electric_backup", end_use="total")

    # [5] Heat pump – min eff + existing backup: all (pkg 5)
    IRA.get_segment_fraction_above_targets(5, "heat_pump_min_eff_with_existing_backup", end_use="total")

    # [6] Heat pump – high eff + basic enclosure: Heating & Cooling (pkg 9)
    IRA.get_segment_fraction_above_targets(9, "heat_pump_high_eff_with_basic_enclosure", end_use="heat_cool")

    # [7] Heat pump – high eff + enhanced enclosure: Heating & Cooling (pkg 10)
    IRA.get_segment_fraction_above_targets(10, "heat_pump_high_eff_with_enhanced_enclosure", end_use="heat_cool")
    ...
    # [10] Heat pump dryer: Clothes dryer (pkg 8, 9, 10)
    IRA.get_segment_fraction_above_targets([8,9,10], "heat_pump_clothes_dryer", end_use="dryer")

    # [11] Electric cooking: Cooking (pkg 7)
    IRA.get_segment_fraction_above_targets(7, "electric_cooking", end_use="cooking")

    # [12] Induction cooking: Cooking (pkg 8, 9, 10)
    IRA.get_segment_fraction_above_targets([8,9,10], "induction_cooking", end_use="cooking")
