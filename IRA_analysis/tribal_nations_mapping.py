...
geography = "puma_tsv" # col name in spatial_tract_lookup <-- "puma_tsv", "county_and_puma"
...
print(f"Creating mapping from ResStock  {resstock_geo}  to 2010 American Indian Areas...")
...
assert len(tract_map) == tract_map["nhgis_2010_tract_gisjoin"].nunique(), \
...
tract_map["percent_hu"] = tract_map["housing_units"].divide(
    tract_map.groupby(geography)["housing_units"].sum()).reindex(tract_map.index)
tract_map = tract_map.reset_index()

tract_map["statefp"] = tract_map["nhgis_2010_tract_gisjoin"].apply(lambda x: int(x[1:3]))
state_map = tract_map[["statefp", "state_abbreviation"]].drop_duplicates().set_index("statefp")["state_abbreviation"]
...
aia_with_units = len(tract_aia.loc[tract_aia["HUINT"]>0][["STATEFP", "NAME"]].drop_duplicates())
...
tract_aia["nhgis_2010_tract_gisjoin"] = "G" + tract_aia["STATEFP"].astype(str).str.zfill(2) + \
    "0" + tract_aia["COUNTYFP"].astype(str).str.zfill(3) + \
    "0" + tract_aia["TRACTCE"].astype(str).str.zfill(6)
...
tract_aia = tract_aia[tract_aia["nhgis_2010_tract_gisjoin"] != "G3600530940103"].reset_index(drop=True)

# QC
tract_delta = set(tract_aia["nhgis_2010_tract_gisjoin"])-set(tract_map["nhgis_2010_tract_gisjoin"])
assert len(tract_delta) == 0, f"AIA crosswalk contains tracts not in {filename.stem}:\n{tract_delta}"


# [3] Join two mappings, retaining only relevant tracts
tract_joined = tract_map[["nhgis_2010_tract_gisjoin"]+geography+["county_name","percent_hu"]].join(
    tract_aia.set_index("nhgis_2010_tract_gisjoin"), on="nhgis_2010_tract_gisjoin", how="right")

# QC
tract_delta = (set(tract_aia["nhgis_2010_tract_gisjoin"].unique()) - 
    set(tract_joined.loc[~tract_joined["PERCENT_HU"].isna(),"nhgis_2010_tract_gisjoin"]))
assert len(tract_delta) ==0, \
...
geography_to_aia = tract_joined.groupby(geography+["AIANNHCE", "NAME", "State"]).apply(
    lambda x: (x["percent_hu"]*x["PERCENT_HU"]).sum()).rename("percent_hu").to_frame()\
    .reset_index().rename(columns=dict(zip(geography, renamed_geography)))


# [5] Optional - Add sampling_probability
opt_sat = pd.read_csv(data_dir / "options_saturations.csv")
opt_sat = opt_sat[opt_sat["Parameter"]==resstock_geo].set_index("Option")["Saturation"]
...
df = (geography_to_aia.groupby(["AIANNHCE", "NAME", "State"]).apply(
    lambda x: (x["sampling_probability"]*x["percent_hu"]).sum())*n_buildings_represented
    ).rename("housing_units").to_frame().reset_index()
