...
def process_ami_lookup(geography):
    ...
    ami_lookup = ami_lookup[~ami_lookup[deps+[income_col]].isna().any(axis=1)].reset_index(drop=True)[deps+[income_col]]
    ...
    if geography == "PUMA":
        ...
        ami_lookup[geography] = ami_lookup[geography].map(puma_map.set_index("puma_tsv")["nhgis_2010_puma_gisjoin"])
...
def assign_representative_income(df):
    ...
    df = df.join(ami_lookup_puma.set_index(deps_puma), on=[geo_col, "in.occupants", "in.federal_poverty_level", "in.income"])
    ...
    df.loc[df["rep_income"].isna(), "rep_income"] = df[keys].astype(str).agg("-".join, axis=1).map(
            ami_lookup_state.assign(key=ami_lookup_state[deps_state].astype(str).agg("-".join, axis=1)).set_index("key")["rep_income"]
            )
...
def add_ami_column_to_file(file_path):
    print("==============================================================================")
    print("This script is for use on ResStock results postprocessed for Sightglass only. ")
    ...
    print("")
    print("This script requires resstock-estimation conda env and access to resstock-estimation GitHub repo")
    ...
    df = POM.map_percent_area_median_income(
            df, "median2019", "rep_income", "rep_occupants", output_col="in.area_median_income"
        )
    assert len(df[df["in.area_median_income"].isna()]) == 0, df[df["in.area_median_income"].isna()]
