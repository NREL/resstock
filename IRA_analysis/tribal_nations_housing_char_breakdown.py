...
# load mappings
mapping = pd.read_csv(data_dir / "2010_derived_map_puma_to_aia.csv").drop(columns=["sampling_probability"])
...
tsv_list2 = ["Vacancy Status", "Federal Poverty Level Tribal", "State", "PUMA", "Tenure", "Geometry Building Type RECS"]
tsv_list3 = ["Vacancy Status", "Area Median Income Tribal", "State", "PUMA", "Tenure", "Geometry Building Type RECS"]
...
for tsv_list, filtering in zip(
    ...
    for idx, tsv_to_pivot in enumerate([tsv_to_pivot1, tsv_to_pivot2]):
        ...
        df.loc[df["Geometry Building Type RECS"].str.startswith("Multi-Family"), new_col] = "Multi-Family"
        ...
        # apply AIA mapping
        df = df.join(mapping.set_index(["PUMA","State"]), on=["PUMA","State"], how="inner")
