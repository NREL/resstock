...
def create_input_tsv(model, dummy_data_file, tsv_file=None):
    ...
    input_options = {
        ...
        "Heating Fuel": ["Electricity", "Fuel Oil", "Natural Gas", "Other Fuel", "Propane", "None"], # old # has special_mapping
        "Water Heater Fuel": ["Electricity", "Fuel Oil", "Natural Gas", "Other Fuel", "Propane"], # has special_mapping
        "Cooking Range": [
            "Electric, 80% Usage", "Gas, 80% Usage", "Propane, 80% Usage",
            "Electric, 100% Usage", "Gas, 100% Usage", "Propane, 100% Usage", 
            "Electric, 120% Usage", "Gas, 120% Usage", "Propane, 120% Usage",
            "None"], # old # has special_mapping
        "Clothes Dryer": [
            "Electric, 80% Usage", "Gas, 80% Usage", "Propane, 80% Usage",
            "Electric, 100% Usage", "Gas, 100% Usage", "Propane, 100% Usage", 
            "Electric, 120% Usage", "Gas, 120% Usage", "Propane, 120% Usage",
    ...
    dfd = pd.read_excel(dummy_data_file, sheet_name=data_input_sheetname, header=0).iloc[:, :-2]
...
def main(filename=None, predict_proba=False, retain_proba=False, validate_model=False, plot_only=False, sfd_only=False, predict_using_model=False):
    ...
    if predict_using_model or not tsv_file.exists():
        # Load model
        feature_names = pd.read_excel(dummy_data_file, sheet_name=data_input_sheetname, header=0, nrows=0).columns.tolist()[:-2]
