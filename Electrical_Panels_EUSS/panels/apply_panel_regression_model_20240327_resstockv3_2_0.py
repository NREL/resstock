""" Clean version, applies to ResStock v3.2.0 
updated: 04/29/24
"""
...
def create_input_tsv(
    ...
    # [2.2] Heating Fuel (Wood = None)
    df_new["Dependency=Heating Fuel"] = "Wood"
    df = pd.concat(
        [df.loc[df["Dependency=Heating Fuel"] != "Wood"], df_new], axis=0
    )
...
def main(
    ...
    # Model specs
    model_num = "41906"  # <-- [41138 (for paper), 41906 (for TEA?, TBD)]
    (
        model_filename,
    ...
    tsv_file = data_dir / f"ResStock v3.2.0 - Electrical Panel Amp - Model {model_num}.tsv"
