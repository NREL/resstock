...
def apply_tsv_to_results(
    ...
    assert (
        dff.loc[cond, panel_labels].isna().sum().sum() == 0
...
def apply_model_to_results(
    ...
    # special mapping for heating fuels:
    dfi["sqft"] = dfi["sqft"].astype(float)
    dfi = apply_special_mapping(dfi, model.model_num)
...
def main(
    ...
    data_dir = local_dir / "model_20240327"
    ...
    dummy_data_file = data_dir / f"model_data_20240327_clean.xlsx"
