...
class Lighting
  def self.apply(runner, model, epw_file, spaces, lighting_groups, lighting, eri_version, schedules_file, cfa, unavailable_periods)
    ...
    if int_kwh > 0
      ...
      if interior_sch.nil?
        interior_unavailable_periods = Schedule.get_unavailable_periods(interior_col_name, unavailable_periods)
    ...
    if grg_kwh > 0
      ...
      if garage_sch.nil?
        garage_unavailable_periods = Schedule.get_unavailable_periods(garage_col_name, unavailable_periods)
    ...
    if ext_kwh > 0
      ...
      if exterior_sch.nil?
        exterior_unavailable_periods = Schedule.get_unavailable_periods(exterior_col_name, unavailable_periods)
    ...
    if not lighting.holiday_kwh_per_day.nil?
      ...
      if exterior_holiday_sch.nil?
        exterior_holiday_unavailable_periods = Schedule.get_unavailable_periods(exterior_holiday_col_name, unavailable_periods)
