...
class AddOSWaterHeaterMixedTanklessGas_Test < Test::Unit::TestCase
  ...
  def setup
    ...
    @measure = AddOSWaterHeaterMixedTanklessGas.new
  ...
  def test_second_argument_is_rated_energy_factor
    ...
    assert_equal "Rated Energy Factor of Gas Tankless Water Heater. This field is ignored for NCTH and B10 protocols.", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_fourth_argument_is_water_heater_location
    ...
    assert_equal "Thermal Zone where the Gas Tankless Water Heater is located", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_water_heater_capacity_must_be_greater_than_0kW
    assert_error_if_arg_less_than(0, "water_heater_capacity", "Gas Tankless Water Heater Nominal Capacity must be greater than 0 kBtu/hr.")
  end
  ...
  def test_water_heater_capacity_of_negative_one_is_an_error
    ...
    errors = @runner.result.errors.collect{ |w| w.logMessage}
    assert_includes errors, "Gas Tankless Water Heater Nominal Capacity must be greater than 0 kBtu/hr."    
  end
  ...
  def test_rated_energy_factor_should_be_in_sensible_range
    assert_warning_if_arg_less_than(0.68, "rated_energy_factor",
                                    "AHRI Certified Energy Factors for Commercially available Gas Tankless Water Heaters should be 0.68 or greater.")
    assert_warning_if_arg_greater_than(0.99, "rated_energy_factor",
                                    "AHRI Certified Energy Factors for Commercially available Gas Tankless Water Heaters should be less than 0.99.", 0.001)
  end
  
  def test_water_heater_capacity_should_be_in_sensible_range
    assert_warning_if_arg_less_than(74.9,"water_heater_capacity",
                                    "Commercially Available Gas Tankless Water Heaters should have a minimum Nominal Capacity of 74.9 kBtu/hr.")
    assert_warning_if_arg_greater_than(199.9,"water_heater_capacity",
                                    "Commercially Available Gas Tankless Water Heaters should have a maximum Nominal Capacity of 199.9 kBtu/hr.",0.001)
  end
  ...
  def test_initial_conditions_include_existing_water_heaters
    ...
    assert_includes initial_condition, "Test Existing Tankless Heater Gas"
    assert_includes initial_condition, "Test Plant Loop 2", "Initial condition should include existing heater's plant loop"
  ...
  def test_water_heater_fuel_is_gas
    set_argument("fuel_type", "Natural Gas")
    @measure.run(@model, @runner, @argument_map)
