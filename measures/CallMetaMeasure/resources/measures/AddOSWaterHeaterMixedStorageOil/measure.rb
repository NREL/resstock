...
#start the measure
class AddOSWaterHeaterMixedStorageOil < OpenStudio::Ruleset::ModelUserScript
    ...
    def description
        return "This measure adds a new residential oil storage water heater to the model based on user inputs. If there is already an existing residential water heater in the model, it is replaced."
    end
  
    def modeler_description
        return "The measure will create a new instance of the OS:WaterHeater:Mixed object representing a oil storage water heater. The measure will be placed on the plant loop 'Domestic Hot Water Loop'. If this loop already exists, any water heater on that loop will be removed and replaced with a water heater consistent with this measure. If it doesn't exist, it will be created."
    end
    ...
    def run(model, runner, user_arguments)
        ...
        model.getPlantLoops.each do |pl|
            if pl.name.to_s == Constants.PlantLoopDomesticWater
                runner.registerInfo("A oil water heater will be added to the existing DHW plant loop")
        ...
        new_heater = Waterheater.create_new_heater(cap, Constants.FuelTypeOil, vol, nbeds, nbaths, ef, re, t_set, water_heater_tz, oncycle_p, offcycle_p, Constants.WaterHeaterTypeTank, 0, model, runner)
    ...
    def validate_storage_tank_volume(vol, runner)
        ...
        if vol > 120
            runner.registerWarning("A residential water heater with a storage tank volume of greater than 120 gallons and a certified rating is not commercially available. Please review the input.")
        end    
        return true
    end
    ...
    def validate_rated_energy_factor(ef, runner)
        if (ef >= 1)
            runner.registerError("Rated energy factor has a maximum value of 1.0 for oil water heaters.")
            return nil
        end
        if (ef <= 0)
    ...
    def validate_water_heater_recovery_efficiency(re, runner)
        if (re < 0)
            runner.registerError("Oil storage water heater recovery efficiency must be at least 0 and at most 1.")
            return nil
        end
        if (re > 1)
            runner.registerError("Oil storage water heater recovery efficiency must be at least 0 and at most 1.")
        ...
        if (re < 0.75)
            runner.registerWarning("Commercially available oil storage water heaters should have a minimum rated recovery efficiency of 0.75.")
        end
        if (re > 0.95)
            runner.registerWarning("Commercially available oil storage water heaters should have a maximum rated recovery efficiency of 0.95.")
        end
        return true
    end
...
AddOSWaterHeaterMixedStorageOil.new.registerWithApplication
