...
class RebuildExistingModel < OpenStudio::Ruleset::ModelUserScript
  ...
  def run(model, runner, user_arguments)
    ...
    bldg_data = retrieve_data_for_sample(resstock_csv, sample_number)

    # Call each measure for sample to build up model
    key_prefix = "res_stock_reporting."
    bldg_data.headers.each do |header|
        next if not header.start_with?(key_prefix)

        prob_dist_file = File.join(resources_dir, "inputs", res_stock_mode, header.sub(key_prefix, "") + ".txt")
  ...
  def retrieve_data_for_sample(resstock_csv, sample_number)
