  ...
  def run(model, runner, user_arguments)
    ...
    model.getSubSurfaces.sort.each do |sub_surface|
      ...
      simple_glazings.each do |simple_glazing|
        ...
        # puts "------------------------------------------------"
        # puts "Thermal Zone Name: #{thermal_zone.name.get}"
        # puts
        # puts "Thermal Zone Size: #{thermal_zone.floorArea}"
        # puts "Window Size: #{sub_surface.grossArea}"
        # puts
        # puts "Daylight Sensor: #{daylight_sensor.name.get}"
        # puts
        # puts "Outside Air Temp Sensor: #{sens_oa_temp}"
        # puts
        # puts "Window Glare Sensor: #{sens_window_glare}"
        # puts
        # puts "Actuator: #{act_window}"
        # puts "------------------------------------------------"
        ...
        window_electrochromic_prg_body = <<-EMS
                ...
                IF #{sens_window_glare.name.get} >= 24,
                    SET #{act_window.name.get} = #{ems_civ_4.name.get},
                ELSEIF (#{sens_window.name.get} * #{shgc_clear} < 180) || (sens_oa_temp < 12.7778),
                    SET #{act_window.name.get} = #{ems_civ_1.name.get},
                ELSEIF #{sens_window.name.get} * #{shgc_2} < 180
                    SET #{act_window.name.get} = #{ems_civ_2.name.get},
                ELSEIF #{sens_window.name.get} * #{shgc_3} < 180
