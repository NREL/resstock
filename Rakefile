...
def copy_resources(branch)
  ...
  extra_files = [
                 ...
                 File.join("measures", "HPXMLTranslator", "resources", "meta_measure.rb") # Needed by buildstock.rb
                ]
...
def copy_measures(beopt_measures_dir, buildstock_resource_measures_dir)
  ...
  Dir.foreach(beopt_measures_dir) do |beopt_measure|
    next if !beopt_measure.include? 'Residential' and beopt_measure != "HPXMLTranslator"
...
def copy_other_measures(beopt_measures_dir)
  ...
  other_measures = ["TimeseriesCSVExport", "ResidentialSimulationControls", "HPXMLTranslator"] # Still under development: "UtilityBillCalculationsSimple", "UtilityBillCalculationsDetailed"
