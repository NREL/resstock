...
def get_example_single_family_detached_values(osw_file, step)
  if ['base-single-family-detached.osw'].include? osw_file
    ...
    step.setArgument("schedules_output_path", File.absolute_path(File.join(File.dirname(__FILE__), "tests", "run", "schedules.csv")))
    ...
    step.setArgument("heating_setback_temp", Constants.DefaultHeatingSetpoint)
    ...
    step.setArgument("cooling_setup_temp", Constants.DefaultCoolingSetpoint)
