...
class Create_DFs():
    ...
    def federal_poverty_level(self):
        df = self.session
        df = df.rename(columns={'metro': 'Dependency=Metro', 'vintage': 'Dependency=Vintage', 'heatingfuel': 'Dependency=Heating Fuel', 'actype': 'Dependency=HVAC System Cooling'})
        df['Dependency=Metro'] = df['Dependency=Metro'].astype(str)
        # df = df.head(5000)
        df, cols = categories_to_columns(df, 'fplbins')
        df = df.groupby(['Dependency=Metro', 'Dependency=Vintage', 'Dependency=Heating Fuel', 'Dependency=HVAC System Cooling'])
        missing_groups = []
        for group in itertools.product(*[['1', '2', '3', '4', '5'], ['<1950', '1950s', '1960s', '1970s', '1980s', '1990s', '2000s'], ['Electricity', 'Fuel Oil', 'Natural Gas', 'Propane', 'Other Fuel', 'None'], ['Central', 'Room', 'None']]):
            if not group in list(df.groups):
                missing_groups.append(dict(zip(['Dependency=Metro', 'Dependency=Vintage', 'Dependency=Heating Fuel', 'Dependency=HVAC System Cooling'], group)))
        ...
        for group in missing_groups:
            df_new = pd.DataFrame(data=dict(group.items() + dict(zip(columns, [1.0/len(columns)] * len(columns))).items()), index=[0]).set_index(['Dependency=Metro', 'Dependency=Vintage', 'Dependency=Heating Fuel', 'Dependency=HVAC System Cooling'])
            df_new['Count'] = 0
            df_new['Weight'] = 0
            df = df.append(df_new)
        ...
        df = df.sort_values(by=['Dependency=Metro', 'Dependency=Vintage', 'Dependency=Heating Fuel', 'Dependency=HVAC System Cooling']).set_index(['Dependency=Metro', 'Dependency=Vintage', 'Dependency=Heating Fuel', 'Dependency=HVAC System Cooling'])
        return df
