...
# load("bs2020_150k.RData") # new buildstock csv with 150,000 units
load("bs2020GHG2.RData") # bs2020_150k including regression energy estimates and GHG calculations. Calculated in bs_combine
load("~/Yale Courses/Research/US Housing/AHS/RenovationStats_new.RData") # moved to new version 12/7/20
rs<-bs2020GHG
nms<-names(rs) # see which columns can be removed, none until after the RS simulations have been done
# base_weight is already included in the rs file
...
rs$change_hren<-rs$change_wren<-rs$change_iren<-rs$change_cren<-0 # maybe not needed. Actually I will use it
rscn<-names(rs)
...
hfeff_types<-rownames(table(rs$Heating.Fuel_Efficiency)) 
# add in extra fuel-efficiency combos that are not currently included
hfeff_types<-c(hfeff_types[1:3],"Electricity_ASHP, SEER 16, 9.0 HSPF","Electricity_ASHP, SEER 18, 9.3 HSPF","Electricity_ASHP, SEER 22, 10 HSPF", # add in three higher efficiency ASHP
               hfeff_types[4:7],"Electricity_MSHP, SEER 14.5, 8.2 HSPF","Electricity_MSHP, SEER 17, 9.5 HSPF","Electricity_MSHP, SEER 25, 12.7 HSPF",# add in three minisplit HP. The third is a new option
               hfeff_types[8:19],"Natural Gas_Fuel Boiler, 90% AFUE","Natural Gas_Fuel Boiler, 96% AFUE",hfeff_types[20:39]) # add in a 90% and 96% AFUE gas boiler
# the new hfeff_types now has 47 options altogether, unlike 51 in the first version of this code. If using a new bscsv, check if there isn't overlap 
...
# adjust heat fuel swithces to reflect higher electrification, 0 oil, and much lower gas
# NorthEast
pfuel_all[,"Electricity","NE"]<-c(0.75,0.1,0.115,0,0,0.035)
pfuel_all[,"Electricity HP","NE"]<-c(0,1,0,0,0,0)
pfuel_all[,"Gas","NE"]<-c(0.025,0.1,0.875,0,0,0) # 10% of gas becomes el HP
pfuel_all[,"Oil","NE"]<-c(0.05,0.15,0.15,0.6,0,0.05) # much quicker scaledown of oil, 10% becomes el HP
pfuel_all[,"Propane","NE"]<-c(0.025,0.1,0.05,0,0,0.825) # 10% of gas becomes el HP
# MidWest
pfuel_all[,"Electricity","MW"]<-c(0.7,0.15,0.125,0,0,0.025) # Faster shift to el HP, smaller role for gas
pfuel_all[,"Electricity HP","MW"]<-c(0,1,0,0,0,0)
pfuel_all[,"Gas","MW"]<-c(0.025,0.075,0.9,0,0,0) # 7.5% of gas becomes el HP
pfuel_all[,"Oil","MW"]<-c(0.15,0.35,0.35,0,0,0.15) # no oil is replaced with oil, large role for HP
pfuel_all[,"Propane","MW"]<-c(0.05,0.1,0.15,0,0,0.7) # 10% of propane becomes el HP
# South
pfuel_all[,"Electricity","S"]<-c(0.75,0.2,0.05,0,0,0) # bigger role for el HP, less gas
pfuel_all[,"Electricity HP","S"]<-c(0,1,0,0,0,0)
pfuel_all[,"Gas","S"]<-c(0.1,0.1,0.8,0,0,0) # 10% of gas becomes el HP and el Res
pfuel_all[,"Oil","S"]<-c(0.35,0.55,0.1,0,0,0) # no oil is replaced with oil, large role for HP
pfuel_all[,"Propane","S"]<-c(0.25,0.15,0.1,0,0,0.5) # much of propane is replaced with electricity
# West
pfuel_all[,"Electricity","W"]<-c(0.7,0.25,0.05,0,0,0.00) # much bigger role for el HP 
pfuel_all[,"Electricity HP","W"]<-c(0,1,0,0,0,0)
...
# adjust water heat fuel swithces to reflect higher electrification, lower oil, and  lower gas
# Northeast
pwfuel_all[,"Electricity","NE"]<-c(0.98,0.02,0,0,0)
pwfuel_all[,"Gas","NE"]<-c(0.15,0.85,0,0,0) # 
pwfuel_all[,"Oil","NE"]<-c(0.3,0.15,0.5,0,0.05) # half of oil is replaced with oil
pwfuel_all[,"Propane","NE"]<-c(0.25,0.15,0,0,0.6) # more propane becomes electric
# Midwest
pwfuel_all[,"Electricity","MW"]<-c(0.95,0.05,0,0,0)
pwfuel_all[,"Gas","MW"]<-c(0.15,0.85,0,0,0) # 
...
pwfuel_all[,"Gas","S"]<-c(0.2,0.8,0,0,0) # 
pwfuel_all[,"Oil","S"]<-c(0.65,0.35,0,0,0) # 
pwfuel_all[,"Propane","S"]<-c(0.25,0.25,0,0,0.5) # Much more propane becomes electric
# West
pwfuel_all[,"Electricity","W"]<-c(1,0,0,0,0) # all electric stays electric
pwfuel_all[,"Gas","W"]<-c(0.15,0.85,0,0,0) # 
pwfuel_all[,"Oil","W"]<-c(0.65,0.35,0,0,0) # 
pwfuel_all[,"Propane","W"]<-c(0.3,0.2,0,0,0.5) # Much more propane becomes electric
...
wins_types<-rownames(table(rs$Insulation.Wall,rs$Geometry.Wall.Type))
wins_types<-wins_types[c(1,6,5,2,3,4,11,10,7,8,9)] # bring the less efficient options in order
wins_types<-c(wins_types,"Wood Stud, R-19, R-5 Sheathing") # add extra option for wall insulation
...
crins_types<-rownames(table(rs$Insulation.Crawlspace))
crins_types<-crins_types[c(4,1,2,3,6,5)]
crins_types<-c(crins_types,"Wall R-15, Unvented")
...
ubins_types<-rownames(table(rs$Insulation.Unfinished.Basement))
ubins_types<-ubins_types[c(4,1,2,3)]
...
uains_types<-rownames(table(rs$Insulation.Unfinished.Attic))
uains_types<-uains_types[c(8,6,1:5,7)]
...
puains[7,6:7]<-1
puains[8,8]<-1 # None remains None
...
# First I will return to the renovation trends to find out fuel switching for water heaters, and see if they note equipment changes to heat pumps. In fact the surveys
# only mention fuel, not technology. If I want I could look at sales data to see trends in new water heating systems
wheff_types<-rownames(table(rs$Water.Heater.Efficiency))
wheff_types<-wheff_types[c(3,2,4,1,9,8,10,7,6,5,11,13,12,14)]
...
ceff_types<-c(ceff_types[c(4,1:3)], "AC, SEER 18",ceff_types[c(5:6,9:10,7:8,11)]) # add in SEER 18 as a central AC option
...
for (rc in 1:4) { # for row/column 1:4
  phfe[rc+1:2,rc]<-pfuel_all["Electricity HP","Electricity HP",RGs[r]]*c(0.25,0.75) # multiply the chance of reamining with a heat pump by 0.5 as we split this into two efficiency levels
}
...
# assign electric standard to el standard/prem/tankless/HP with 80:10:5:5 (adv) odds
whfe[1:4,"Electric Standard"]<-c(0.8,0.1,0.05,0.05)*pwfuel_all["Electricity","Electricity",RGs[r]]
...
# oil standard to elec with 80:10:5:5 odds
whfe[1:4,"Fuel Oil Standard"]<-c(0.8,0.1,0.05,0.05)*pwfuel_all["Electricity","Oil",RGs[r]]
...
# propane standard to elec with  80:10:5:5 odds
whfe[1:4,"Propane Standard"]<-c(0.8,0.1,0.05,0.05)*pwfuel_all["Electricity","Propane",RGs[r]]
...
#   labs(title = "Electric heating systems by equipment type and efficiency, 2020-2060", y = "Sampled Units",caption = "Total Sample = 150,000")
...
#   labs(title = "Gas heating systems by equipment type and efficiency, 2020-2060", y = "Sampled Units",caption = "Total Sample = 150,000")
# 
...
#   labs(title = "Oil heating systems by equipment type and efficiency, 2020-2060", y = "Sampled Units",caption = "Total Sample = 150,000")
# 
# windows()
# ggplot(hfe_el,aes(x=Year,y=Freq,group=HeatEfficiency))+geom_line(aes(color=HeatEfficiency),size=1) + scale_y_continuous(labels = scales::comma) + theme_bw() #+ scale_fill_brewer(palette="Paired")
# 
...
#   labs(title = "2020 housing units by Wall Insulation, 2020-2060", y = "Sampled Units",caption = "Total Sample = 150,000")
...
#   labs(title = "2020 housing units by AC Type, 2020-2060", y = "Sampled Units",caption = "Total Sample = 150,000")
# 
...
# save(rs_2020_2060,file="RenAdvanced2.RData")
...
for (yr in c(seq(2025,2060,5))) {
  ...
  dd$hv # don't know what this is about or if somethig was accidentally deleted
  ...
  # characterization of renovations updated on Feb 7, larger reductions assumed to reflect the deeper renovations
  dd[dd$change_hren>0&dd$HVAC.Heating.Type.And.Fuel %in% c("Electricity ASHP","Electricity MSHP"),]$Elec_MJ_new<-
    round(0.9*dd[dd$change_hren>0&dd$HVAC.Heating.Type.And.Fuel %in% c("Electricity ASHP","Electricity MSHP"),]$Elec_MJ_new) # if a heating system involved upgrading to a (more efficient) heat pump, reduce electricity consumption by 8%
  
  dd[dd$change_hren>0&dd$Heating.Fuel=="Natural Gas",]$NaturalGas_MJ_new<-round(0.93*dd[dd$change_hren>0&dd$Heating.Fuel=="Natural Gas",]$NaturalGas_MJ_new) # reduction from upgrade to a new gas heating system
  dd[dd$change_hren>0&dd$Heating.Fuel=="Propane",]$Propane_MJ_new<-round(0.93*dd[dd$change_hren>0&dd$Heating.Fuel=="Propane",]$Propane_MJ_new) # reduction from upgrade to a new propane heating system
  dd[dd$change_hren>0&dd$Heating.Fuel=="Fuel Oil",]$FuelOil_MJ_new<-round(0.86*dd[dd$change_hren>0&dd$Heating.Fuel=="Fuel Oil",]$FuelOil_MJ_new) # reduction from upgrade to a new FO heating system
  
  dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Electricity",]$Elec_MJ_new<-round(0.95*dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Electricity",]$Elec_MJ_new) # reduction from upgrade to new elec water heating system
  dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Natural Gas",]$NaturalGas_MJ_new<-round(0.93*dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Natural Gas",]$NaturalGas_MJ_new) # reduction from upgrade to new gas water heating system
  dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Propane",]$Propane_MJ_new<-round(0.93*dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Propane",]$Propane_MJ_new) # reduction from upgrade to new prop water heating system
  dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Fuel Oil",]$FuelOil_MJ_new<-round(0.95*dd[dd$change_wren>0&dd$Water.Heater.Fuel=="Fuel Oil",]$FuelOil_MJ_new) # reduction from upgrade to new FO water heating system
  
  dd[dd$change_iren>0&dd$Heating.Fuel=="Electricity",]$Elec_MJ_new<-round(0.96*dd[dd$change_iren>0&dd$Heating.Fuel=="Electricity",]$Elec_MJ_new) # reduction from upgrade to insulation
  dd[dd$change_iren>0&dd$Heating.Fuel=="Natural Gas",]$NaturalGas_MJ_new<-round(0.85*dd[dd$change_iren>0&dd$Heating.Fuel=="Natural Gas",]$NaturalGas_MJ_new) # reduction from upgrade to insulation
  dd[dd$change_iren>0&dd$Heating.Fuel=="Propane",]$Propane_MJ_new<-round(0.85*dd[dd$change_iren>0&dd$Heating.Fuel=="Propane",]$Propane_MJ_new) # reduction from upgrade to insulation
  dd[dd$change_iren>0&dd$Heating.Fuel=="Fuel Oil",]$FuelOil_MJ_new<-round(0.8*dd[dd$change_iren>0&dd$Heating.Fuel=="Fuel Oil",]$FuelOil_MJ_new) # reduction from upgrade to insulation
...
wfe$WaterFuel_Efficiency<-gsub("FIXME ","",wfe$WaterFuel_Efficiency)
windows()
...
cols<-colorRampPalette(brewer.pal(10,"Paired"))(length(unique(wfe$WaterFuel_Efficiency)))
windows()
...
hfe_el<-hfe[hfe$HeatFuel=="Electricity",]
cols<-colorRampPalette(brewer.pal(10,"Paired"))(length(unique(hfe_el$HeatEfficiency)))
windows()
ggplot(hfe_el,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma)+ theme_bw() +scale_fill_manual(values = cols) +
  labs(title = "Pre-2020 housing units with electricity systems, by type and efficiency, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Advanced Renovation",sep=""))
...
hfe_gas<-hfe[hfe$HeatFuel=="Natural Gas",]
cols<-colorRampPalette(brewer.pal(10,"Paired"))(length(unique(hfe_gas$HeatEfficiency)))
windows()
ggplot(hfe_gas,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma)+ theme_bw() +scale_fill_manual(values = cols) +
  labs(title = "Pre-2020 housing units with gas systems, by type and efficiency, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Advanced Renovation",sep=""))
...
ggplot(hfe_oil,aes(x=Year,y=1e-6*Count,fill=HeatEfficiency))+geom_bar(position="stack", stat="identity") + scale_y_continuous(labels = scales::comma)+ theme_bw() +scale_fill_manual(values = cols) +
  labs(title = "Pre-2020 housing units with oil systems, by type and efficiency, 2020-2060", y = "Million Housing Units",subtitle = paste(scenario_names[sts], ", Advanced Renovation",sep=""))
...
GHG_base_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHG_base$GHG,GHG_base$Year,sum))
GHG_base_p2020<-data.frame(with(select(GHG_base_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHG_base_p2020)=c("Year","GHG")

GHG_hiDR_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHG_hiDR$GHG,GHG_hiDR$Year,sum))
GHG_hiDR_p2020<-data.frame(with(select(GHG_hiDR_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHG_hiDR_p2020)=c("Year","GHG")

GHG_hiMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHG_hiMF$GHG,GHG_hiMF$Year,sum))
GHG_hiMF_p2020<-data.frame(with(select(GHG_hiMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHG_hiMF_p2020)=c("Year","GHG")

GHG_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHG_hiDRMF$GHG,GHG_hiDRMF$Year,sum))
GHG_hiDRMF_p2020<-data.frame(with(select(GHG_hiDRMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHG_hiDRMF_p2020)=c("Year","GHG")

GHGel_base_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGel_base$GHG,GHGel_base$Year,sum))
GHGel_base_p2020<-data.frame(with(select(GHGel_base_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGel_base_p2020)=c("Year","GHG")

GHGel_hiDR_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGel_hiDR$GHG,GHGel_hiDR$Year,sum))
GHGel_hiDR_p2020<-data.frame(with(select(GHGel_hiDR_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGel_hiDR_p2020)=c("Year","GHG")

GHGel_hiMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGel_hiMF$GHG,GHGel_hiMF$Year,sum))
GHGel_hiMF_p2020<-data.frame(with(select(GHGel_hiMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGel_hiMF_p2020)=c("Year","GHG")

GHGel_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGel_hiDRMF$GHG,GHGel_hiDRMF$Year,sum))
GHGel_hiDRMF_p2020<-data.frame(with(select(GHGel_hiDRMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGel_hiDRMF_p2020)=c("Year","GHG")

GHGng_base_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGng_base$GHG,GHGng_base$Year,sum))
GHGng_base_p2020<-data.frame(with(select(GHGng_base_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGng_base_p2020)=c("Year","GHG")

GHGng_hiDR_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGng_hiDR$GHG,GHGng_hiDR$Year,sum))
GHGng_hiDR_p2020<-data.frame(with(select(GHGng_hiDR_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGng_hiDR_p2020)=c("Year","GHG")

GHGng_hiMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGng_hiMF$GHG,GHGng_hiMF$Year,sum))
GHGng_hiMF_p2020<-data.frame(with(select(GHGng_hiMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGng_hiMF_p2020)=c("Year","GHG")

GHGng_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGng_hiDRMF$GHG,GHGng_hiDRMF$Year,sum))
GHGng_hiDRMF_p2020<-data.frame(with(select(GHGng_hiDRMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGng_hiDRMF_p2020)=c("Year","GHG")

GHGfo_base_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGfo_base$GHG,GHGfo_base$Year,sum))
GHGfo_base_p2020<-data.frame(with(select(GHGfo_base_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGfo_base_p2020)=c("Year","GHG")

GHGfo_hiDR_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGfo_hiDR$GHG,GHGfo_hiDR$Year,sum))
GHGfo_hiDR_p2020<-data.frame(with(select(GHGfo_hiDR_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGfo_hiDR_p2020)=c("Year","GHG")

GHGfo_hiMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGfo_hiMF$GHG,GHGfo_hiMF$Year,sum))
GHGfo_hiMF_p2020<-data.frame(with(select(GHGfo_hiMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGfo_hiMF_p2020)=c("Year","GHG")

GHGfo_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGfo_hiDRMF$GHG,GHGfo_hiDRMF$Year,sum))
GHGfo_hiDRMF_p2020<-data.frame(with(select(GHGfo_hiDRMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGfo_hiDRMF_p2020)=c("Year","GHG")

GHGpr_base_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGpr_base$GHG,GHGpr_base$Year,sum))
GHGpr_base_p2020<-data.frame(with(select(GHGpr_base_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGpr_base_p2020)=c("Year","GHG")

GHGpr_hiDR_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGpr_hiDR$GHG,GHGpr_hiDR$Year,sum))
GHGpr_hiDR_p2020<-data.frame(with(select(GHGpr_hiDR_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGpr_hiDR_p2020)=c("Year","GHG")

GHGpr_hiMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGpr_hiMF$GHG,GHGpr_hiMF$Year,sum))
GHGpr_hiMF_p2020<-data.frame(with(select(GHGpr_hiMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGpr_hiMF_p2020)=c("Year","GHG")

GHGpr_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),GHG=tapply(GHGpr_hiDRMF$GHG,GHGpr_hiDRMF$Year,sum))
GHGpr_hiDRMF_p2020<-data.frame(with(select(GHGpr_hiDRMF_ann,Year,GHG),spline(Year,GHG,xout = 2020:2060)),method="spline()")[,1:2]
names(GHGpr_hiDRMF_p2020)=c("Year","GHG")


E_base_ann<-data.frame(Year=seq(2020,2060,5),En=tapply(En_base$Energy,En_base$Year,sum))
E_base_p2020<-data.frame(with(select(E_base_ann,Year,En),spline(Year,En,xout = 2020:2060)),method="spline()")[,1:2]
names(E_base_p2020)=c("Year","Energy")

E_hiDR_ann<-data.frame(Year=seq(2020,2060,5),En=tapply(En_hiDR$Energy,En_hiDR$Year,sum))
E_hiDR_p2020<-data.frame(with(select(E_hiDR_ann,Year,En),spline(Year,En,xout = 2020:2060)),method="spline()")[,1:2]
names(E_hiDR_p2020)=c("Year","Energy")

E_hiMF_ann<-data.frame(Year=seq(2020,2060,5),En=tapply(En_hiMF$Energy,En_hiMF$Year,sum))
E_hiMF_p2020<-data.frame(with(select(E_hiMF_ann,Year,En),spline(Year,En,xout = 2020:2060)),method="spline()")[,1:2]
names(E_hiMF_p2020)=c("Year","Energy")

E_hiDRMF_ann<-data.frame(Year=seq(2020,2060,5),En=tapply(En_hiDRMF$Energy,En_hiDRMF$Year,sum))
E_hiDRMF_p2020<-data.frame(with(select(E_hiDRMF_ann,Year,En),spline(Year,En,xout = 2020:2060)),method="spline()")[,1:2]
names(E_hiDRMF_p2020)=c("Year","Energy")

save(GHG_base_p2020,GHG_hiDR_p2020,GHG_hiMF_p2020,GHG_hiDRMF_p2020,
     GHGel_base_p2020,GHGel_hiDR_p2020,GHGel_hiMF_p2020,GHGel_hiDRMF_p2020,
     GHGng_base_p2020,GHGng_hiDR_p2020,GHGng_hiMF_p2020,GHGng_hiDRMF_p2020,
     GHGfo_base_p2020,GHGfo_hiDR_p2020,GHGfo_hiMF_p2020,GHGfo_hiDRMF_p2020,
     GHGpr_base_p2020,GHGpr_hiDR_p2020,GHGpr_hiMF_p2020,GHGpr_hiDRMF_p2020,
     E_base_p2020,E_hiDR_p2020,E_hiMF_p2020,E_hiDRMF_p2020,
     df_base,df_hiDR,df_hiMF,df_hiDRMF,file = "EG_AdvRen_Summary3.RData")
...
#      GHGpr_base_p2020,GHGpr_hiDR_p2020,GHGpr_hiMF_p2020,GHGpr_hiDRMF_p2020,
#      E_base_p2020,E_hiDR_p2020,E_hiMF_p2020,E_hiDRMF_p2020,
#      df_base,df_hiDR,df_hiMF,df_hiDRMF,file = "EG_AdvRen_Summary_LREC3.RData")
