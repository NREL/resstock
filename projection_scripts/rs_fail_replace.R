# this is a script to identify failed simulations in the resstock results files, and replace them with successful simlulations
...
# Purpose: Replace some simulations which failed due to incompatible combinations of garages and hot water fuel. Replacements of failed TX county files was originally done here also for 2030_RR and 2060_RR but that is now done elsewhere. Now they are only fixed here for 2020

# Inputs: - scen_bscsv/bs2020_180k.csv
#         - Eagle_outputs/RawResults/results_2020.csv
#         - Eagle_outputs/RawResults/res_2020_fail_success.csv, redo of 2020 files which initially failed and succeeded second time around
#         - scen_bscsv_sim/bs_RR_2030.csv
#         - Eagle_outputs/RawResults/res_RR_2030.csv
#         - Eagle_outputs/RawResults/res_RR_2030_fail_success.csv, redo of 2030_RR files which initially failed and succeeded second time around
#         - scen_bscsv_sim/bs_RR_2060.csv
#         - Eagle_outputs/RawResults/res_RR_2060.csv
#         - Eagle_outputs/RawResults/res_RR_2060_fail_success.csv, redo of 2030_RR files which initially failed and succeeded second time around


# Outputs: 
#         - Eagle_outputs/res_2020_complete.csv
#         - Eagle_outputs/res_RR_2030_complete.csv (renamed without the _complete suffix)
#         - Eagle_outputs/res_RR_2060_complete.csv (renamed without the _complete suffix)
...
# # Now fix the TX sims ######## this was removed, it will be done in replacing the complete renovation history in the RS_results

fail_fixed2<-failed[which(failed$building_id %in% f),]

failed_save<-rm_dot2(failed)

write.csv(failed_save,file="../Eagle_outputs/res_RR_2030_complete.csv",row.names = FALSE) # this ended up being renamed without the _complete suffix
...
write.csv(failed_save,file="../Eagle_outputs/res_RR_2060_complete.csv",row.names = FALSE) # this ended up being renamed without the _complete suffix
