...
# write.table(cty,'../resstock_scenarios/project_national_2020/housing_characteristics/County.tsv',append = FALSE,quote = FALSE, row.names = FALSE, col.names = TRUE,sep='\t')
# write.table(ie,'../resstock_scenarios/project_national_2020/housing_characteristics/ASHRAE IECC Climate Zone 2004.tsv',append = FALSE,quote = FALSE, row.names = FALSE, col.names = TRUE,sep='\t')

# now run for scenarios 2025-2060 ############
load("County_Scenario_SM_Results3.RData") # updated to new outputs on Jan 12, and again to #3 on Feb 2
...
for (s in 1:32) {
  ...
  # With the new approach on modeling stock growth and decline for nc cohorts after their decade officially finishes, I want to represent here only nc cohort construction that happends within the appropriate decade.
  ...
  # this should be unccessary now
  # hstock[hstock$GeoID==22059,]$RS_ID<-"LA, La Salle Parish"
  # hstock[hstock$GeoID==35013,]$RS_ID<-"NM, Dona Ana County"
  ...
  rownames(tc_puma)<-rownames(tc_pumaold)
...
puma_i<-puma_list[i]
ptc<-vintage[vintage$Dependency.PUMA==puma_i,] # puma matrix of units by type and cohort. For 2020 stock, only 9 vintages
...
vin<-format(vintage_new,nsmall=6,digits=1,scientific=FALSE)
ty<-format(type_new,nsmall=6,digits=1,scientific=FALSE)
fol<-paste('../resstock_scenarios/project_national_',scen,sep="")
