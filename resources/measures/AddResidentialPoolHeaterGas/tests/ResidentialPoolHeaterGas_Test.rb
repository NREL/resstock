...
class NewMeasureTest < MiniTest::Test
  ...
  def test_new_construction_gas
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(222.0, 3, 2000))
  end
  ...
  def test_new_construction_mult_0_014
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(3.108, 3, 2000))
  end
  ...
  def test_new_construction_no_scale_energy
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, 222.0)
  end

  def test_retrofit_replace
    args_hash = {}
    args_hash["base_energy"] = 222.0
    model = _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(222.0, 3, 2000))
    args_hash = {}
    args_hash["base_energy"] = 111.0
    _test_retrofit(model, args_hash, true, _scale_energy(111.0, 3, 2000))
  end
  
  def test_retrofit_replace_electric_pool_heater
    model = _get_model("2000sqft_2story_FB_GRG_UA_3Beds_2Baths_ElecPoolHeater.osm")
    args_hash = {}
    args_hash["base_energy"] = 111.0
    _test_retrofit(model, args_hash, true, _scale_energy(111.0, 3, 2000))
  end
  ...
  def _test_new_construction(osm_file, args_hash, expected_new_object, expected_annual_therm=nil)
    ...
    if expected_new_object
        ...
        assert_equal(new_object.name.to_s, Constants.ObjectNamePoolHeater(Constants.FuelTypeGas))
  ...
  def _test_retrofit(model, args_hash, expected_new_object, expected_annual_therm=nil)
    ...
    if not expected_new_object
        ...
        assert([Constants.ObjectNamePoolHeater(Constants.FuelTypeElectric), Constants.ObjectNamePoolHeater(Constants.FuelTypeGas)].include?(del_object.name.to_s))
