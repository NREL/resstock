...
class Airflow
  ...
  def self.process_mech_vent_for_unit(model, runner, obj_name_mech_vent, unit, is_existing_home, ela, mech_vent, building, nbeds, nbaths, weather, unit_ffa, unit_living, num_units, has_forced_air_equipment)
    ...
    # whf_fan_power = 0.4 # W/cfm/fan, per CEC appliance complinace sheet's max value
    whf_fan_power = 0.3 # W/cfm/fan, per HSP 
    # whf_fan_power = 0.22 # W/cfm/fan, per CEC appliance compliance sheet's max value after removing outliers
    # whf_fan_power = 0.12 # W/cfm/fan, per CEC appliance compliance sheet's mean/median value
    whf_exhaust_operation = 24 * 60.0 # min/day, all day operation
    ...
    whf_avg_exhaust_cfm = mech_vent.whf_exhaust * whf_exhaust_operation / 24 * 60 # cfm # todo: ask Joe about the 60 or 24 * 60? 
  ...
  def self.create_infil_mech_vent_objects(model, runner, obj_name_infil, obj_name_mech_vent, unit_living, infil, mech_vent, wind_speed, mv_output, infil_output, tin_sensor, tout_sensor, vwind_sensor, duct_lks, cfis_programs, nbeds)
    ...
    if mv_output.has_whf and mech_vent.whf_exhaust > 0
    	infil_program.addLine("Set Qwhf = #{whf_sch_sensor.name}*#{UnitConversions.convert(mv_output.whf_avg_exhaust_cfm, "cfm", "m^3/s").round(4)}")
    else
    	infil_program.addLine("Set Qwhf = 0.0")
		end
    ...
    if mech_vent.type == Constants.VentTypeBalanced
      ...
      if mech_vent.type == Constants.VentTypeExhaust
        infil_program.addLine("Set Qout = QWHV+Qrange+Qbath+Qdryer+Qwhf+QhpwhOut+QductsOut")
        infil_program.addLine("Set Qin = QhpwhIn+QductsIn")
        infil_program.addLine("Set Qu = (@Abs (Qout-Qin))")
...
class MechanicalVentilationOutput
  def initialize(frac_fan_heat, num_fans, whole_house_vent_rate, bathroom_hour_avg_exhaust, range_hood_hour_avg_exhaust, spot_fan_power, latent_effectiveness, sensible_effectiveness, dryer_exhaust_day_shift, has_dryer, whf_fan_power, whf_avg_exhaust_cfm, has_whf, num_fans_whf)
    ...
    @has_dryer = has_dryer
    @whf_fan_power = whf_fan_power
    @whf_avg_exhaust_cfm = whf_avg_exhaust_cfm
    @has_whf = has_whf
    @num_fans_whf = num_fans_whf
  end
  attr_accessor(:frac_fan_heat, :num_fans, :whole_house_vent_rate, :bathroom_hour_avg_exhaust, :range_hood_hour_avg_exhaust, :spot_fan_power, :latent_effectiveness, :sensible_effectiveness, :dryer_exhaust_day_shift, :has_dryer, :whf_fan_power, :whf_avg_exhaust_cfm, :has_whf, :num_fans_whf)
end
