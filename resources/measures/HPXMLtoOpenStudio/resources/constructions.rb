...
class ThermalMassConstructions
  ...
  def self.apply_partition_walls(runner, model, surfaces, constr_name,
    ...
    spaces.each do |space|
      ...
      next unless addtl_surface_area > 0
  ...
  def self.get_walls(model, runner)
    ...
    model.getSpaces.each do |space|
      ...
      space.surfaces.each do |surface|
        ...
        elsif Geometry.is_finished_basement(space) && obc_is_foundation
          surfaces[Constants.SurfaceTypeWallFndGrndFinB] << surface

        # Exterior unfinished basement
        elsif Geometry.is_unfinished_basement(space) && obc_is_foundation
          surfaces[Constants.SurfaceTypeWallFndGrndUnfinB] << surface

        # Exterior crawlspace
        elsif Geometry.is_crawl(space) && obc_is_foundation
  ...
  def self.get_floors(model, runner)
    ...
    model.getSpaces.each do |space|
      ...
      space.surfaces.each do |surface|
        ...
        elsif (not is_finished) && obc_is_adjacent && (not Geometry.space_is_finished(adjacent_space))
        ...
        # Finished slab
        elsif above_grade && is_finished && obc_is_foundation
          surfaces[Constants.SurfaceTypeFloorFndGrndFinSlab] << surface

        # Unfinished slab
        elsif above_grade && (not is_finished) && obc_is_foundation
