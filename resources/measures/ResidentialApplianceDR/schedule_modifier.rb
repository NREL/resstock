...
def shift_peak_to_take(sch, peak_period, take_period, timeClass)
  puts("Dodge peak simple got schedule #{sch.times.map{|x| x.to_s}} and #{sch.values}")
  ...
  times.each_with_index do |time, index|
    ...
    if (time.totalMinutes >= peak_period[0]*60) and (not entered_peak)
      prev_index = index-1
      new_sch_times = times[0, prev_index+1] 
      ...
      puts("Peakclearing: Processing #{time.to_s} which entered peak period. Starting with #{new_sch_times}, #{new_sch_vals}")
    end
  ...
  new_sch_times.each_with_index do |time, index|
    ...
    puts("Processing #{time.to_s}, #{cur_val}. So far built this schedule: #{take_added_sch_time}, #{take_added_sch_vals}")
    if time.totalMinutes >= take_period[0]*60 and (time.totalMinutes < take_period[1]*60 or not entered)
      ...
      puts("Time #{time.to_s} is at or past the start of the take_period. We built #{take_added_sch_time}, #{take_added_sch_vals}")
      puts("Ok...")
    end
    if time.totalMinutes >= take_period[1]*60
      if time.totalMinutes == take_period[1]*60
      ...
      puts("Time #{time.to_s} is at or past the end of the take_period start. We built #{take_added_sch_time}, #{take_added_sch_vals}")
      break
    end
    if time.totalMinutes < take_period[0]*60
      take_added_sch_time << time
