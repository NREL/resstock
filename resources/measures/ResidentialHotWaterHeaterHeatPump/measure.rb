...
class ResidentialHotWaterHeaterHeatPump < OpenStudio::Measure::ModelMeasure
    ...
    def arguments(model)
        ...
        operating_mode_type_args << Constants.Constant
        operating_mode_type_args << Constants.Scheduled
        ...
        operating_mode_type.setDescription("The water heater operating mode type. Constant will use a constant control strategy for the whole year, while scheduled will use 8760 values in a schedule file.")
        operating_mode_type.setDefaultValue(Constants.Constant)
        args << operating_mode_type
        
        # make an argument for operating mode (standard or hp_only)
        operating_mode_args = OpenStudio::StringVector.new
        operating_mode_args << "standard"
        operating_mode_args << "hp_only"
        ...
        operating_mode.setDescription("The water heater operating mode. hp_only only uses the heat pump, while standard allows the backup electric resistance to come on in high demand situations. This is ignored if a scheduled operating mode type is selected.")
        operating_mode.setDefaultValue("standard")
        args << operating_mode
    ...
    def run(model, runner, user_arguments)
        ...
        operating_mode_type = runner.getStringArgumentValue("operating_mode_type",user_arguments)
        ...
        if setpoint_type == Constants.Scheduled or operating_mode_type == Constants.Scheduled
            ...
            if operating_mode_type == Constants.Scheduled
                operating_mode_schedule_file = File.join(schedule_directory, operating_mode_schedule)
                if not File.exists?(operating_mode_schedule_file)
        ...
        units.each_with_index do |unit, unit_index|
            ...
            success = Waterheater.apply_heatpump(model, unit, runner, space, weather,
                                                 ...
                                                 operating_mode_type, setpoint_schedule_file, 
