...
class NewMeasureTest < MiniTest::Test
  ...
  def test_new_construction_electric
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(1027.3, 3, 2000))
  end
  ...
  def test_new_construction_mult_0_048
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(49.3104, 3, 2000))
  end
  ...
  def test_new_construction_no_scale_energy
    ...
    _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, 1027.3)
  end

  def test_retrofit_replace
    args_hash = {}
    args_hash["base_energy"] = 1027.3
    model = _test_new_construction("2000sqft_2story_FB_GRG_UA_3Beds_2Baths.osm", args_hash, true, _scale_energy(1027.3, 3, 2000))
    args_hash = {}
    args_hash["base_energy"] = 555.0
    _test_retrofit(model, args_hash, true, _scale_energy(555.0, 3, 2000))
  end
  
  def test_retrofit_replace_gas_hot_tub_heater
    model = _get_model("2000sqft_2story_FB_GRG_UA_3Beds_2Baths_GasHotTubHeater.osm")
    args_hash = {}
    args_hash["base_energy"] = 555.0
    _test_retrofit(model, args_hash, true, _scale_energy(555.0, 3, 2000))
  end
  ...
  def _test_error(osm_file, args_hash)
    ...
    measure = ResidentialHotTubHeater.new
  ...
  def _test_new_construction(osm_file, args_hash, expected_new_object, expected_annual_kwh=nil)
    ...
    if expected_new_object
        ...
        assert_equal(new_object.name.to_s, Constants.ObjectNameHotTubHeater(Constants.FuelTypeElectric))
  ...
  def _test_retrofit(model, args_hash, expected_new_object, expected_annual_kwh=nil)
    ...
    if not expected_new_object
        ...
        assert([Constants.ObjectNameHotTubHeater(Constants.FuelTypeElectric), Constants.ObjectNameHotTubHeater(Constants.FuelTypeGas)].include?(del_object.name.to_s))
