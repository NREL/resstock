...
#start the measure
class AddOSWaterHeaterMixedTanklessGas < OpenStudio::Ruleset::ModelUserScript
    ...
    def description
        return "This measure adds a new residential gas tankless water heater to the model based on user inputs. If there is already an existing residential water heater in the model, it is replaced."
    end
  
    def modeler_description
        return "The measure will create a new instance of the OS:WaterHeater:Mixed object representing a gas tankless water heater. The measure will be placed on the plant loop 'Domestic Hot Water Loop'. If this loop already exists, any water heater on that loop will be removed and replaced with a water heater consistent with this measure. If it doesn't exist, it will be created."
    end
    ...
    def run(model, runner, user_arguments)
        ...
        model.getPlantLoops.each do |pl|
            if pl.name.to_s == Constants.PlantLoopDomesticWater
                runner.registerInfo("A gas tankless water heater will be added to the existing DHW plant loop")
        ...
        new_heater = Waterheater.create_new_heater(cap, Constants.FuelTypeGas, 1, nbeds, nbaths, ef, 0, t_set, water_heater_tz, oncycle_p, offcycle_p, tanktype, cd, model, runner)
    ...
    def validate_rated_energy_factor(ef, runner)
        ...
        if (ef <0.82)
            runner.registerWarning("Rated energy factor for commercially available gas tankless water heaters should be greater than 0.82")
        end    
        return true
    end
    ...
    def validate_water_heater_capacity(cap, runner)
        ...
        if cap < 120
            runner.registerWarning("Commercially available residential gas tankless water heaters should have a nominal capacity greater than 120 kBtu/h.")
        end
        return true
    end
...
AddOSWaterHeaterMixedTanklessGas.new.registerWithApplication
