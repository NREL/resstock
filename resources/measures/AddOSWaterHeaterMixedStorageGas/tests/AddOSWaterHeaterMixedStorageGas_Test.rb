...
class AddOSWaterHeaterMixedStorageGas_Test < Test::Unit::TestCase
  ...
  def test_first_argument_is_use_case
    arg = @arguments[0]
    assert_equal "use_case", arg.name
  ...
  def test_second_argument_is_number_of_bedrooms
    ...
    refute arg.hasDefaultValue
    choices = arg.choiceValues.collect{ |c| c.to_i}.sort
    assert_equal [1,2,3,4,5], choices
  end
  ...
  def test_fifth_argument_is_storage_tank_volume
    ...
    assert_equal "Volume of the Storage Tank (gallons) of the Gas Hot Water Heater. Set to 0 to have Storage tank volume autosized. This field is ignored for NCTH and B10 protocols.", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_seventh_argument_is_water_heater_location
    ...
    assert_equal "Thermal Zone where the Gas Storage Water Heater is located", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_eight_argument_is_water_heater_capacity
    arg = @arguments[7]
    refute_nil arg
    assert_equal "water_heater_capacity", arg.name
    assert_equal "The nominal capacity [kBtu/hr] of the gas storage water heater. Set to 0 to have this field autosized. This field is ignored for NCTH and B10 protocols.", arg.displayName
    refute arg.hasDefaultValue
    assert_equal 1, arg.type.value
  end
  
  # Argument 9: rated_energy_factor
  
  def test_ninth_argument_is_rated_energy_factor
    arg = @arguments[8]
    assert_equal "rated_energy_factor", arg.name
    assert_equal "Rated Energy Factor of Gas Storage Tank Water Heater. This field is ignored for NCTH and B10 protocols.", arg.displayName
    refute arg.hasDefaultValue
    assert_equal 1, arg.type.value # DoubleArgument
  end
  
  #argument 10: water_heater_recovery_efficiency
  def test_tenth_argument_is_water_heater_recovery_efficiency
    arg = @arguments[9]
    refute_nil arg
    assert_equal "water_heater_recovery_efficiency", arg.name
    assert_equal "Rated Recovery Efficiency of the water heater equal to the ratio of energy delivered to the water to the energy content of the fuel consumed by the water heater. Test procedures to test recovery efficiency are defined by the DOE in 10 CFR Part 430, Appendix E to Subpart B. Enter a number between 0 and 1.0. This is used to calculate the thermal efficiency. Users wil need to review the E+out.eio file to confirm Calculated recovery efficiency - Rated recovery efficiency.", arg.displayName
    assert arg.hasDefaultValue
    assert_equal 0.80, arg.defaultValueAsDouble
  end
  
  # Argument 11: water_heater_fuel_type
  
  def test_eleventh_argument_is_fuel_type
    arg = @arguments[10]
    refute_nil arg
    assert_equal "water_heater_fuel_type", arg.name
    assert_equal "The Type of Fuel used for heating.", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_storage_tank_must_be_positive
    assert_error_if_arg_less_than(0, "storage_tank_volume", "Storage Tank Volume cannot be less than 0 gallons. Please correct.")
  end
  ...
  def test_storage_tank_at_negative_1_is_an_error
    ...
    assert_includes errors, "Storage Tank Volume cannot be less than 0 gallons. Please correct."
  end    
  ...
  def test_water_heater_capacity_must_be_greater_than_0kW
    assert_error_if_arg_less_than(0, "water_heater_capacity", "Gas Storage Water Heater Nominal Capacity must be greater than 0 kBtu/hr.")
  end
  ...
  def test_water_heater_capacity_of_negative_one_is_an_error
    ...
    errors = @runner.result.errors.collect{ |w| w.logMessage}
    assert_includes errors, "Gas Storage Water Heater Nominal Capacity must be greater than 0 kBtu/hr."
  end
    ...
    assert_error_if_arg_less_than(0,
                                  ...
                                  "Gas Storage Water Heater Recovery Efficiency must be at least 0 and at most 1.")
  end
  ...
  def test_water_heater_recovery_efficiency_must_be_greater_than_rated_energy_factor
    set_argument("rated_energy_factor", 0.75)

    # Note: delta for next test is 0.01, so this tests 0.755 OK, 0.745 error
    assert_error_if_arg_less_than(0.755,"water_heater_recovery_efficiency", "Energy factor must be less than Recovery efficiency.")

    set_argument("water_heater_recovery_efficiency", 0.75)

    @measure.run(@model, @runner, @argument_map)
    errors = @runner.result.errors.collect{ |w| w.logMessage}
    assert_includes errors, "Energy factor must be less than Recovery efficiency."
  end
  ...
  def test_warning_if_small_storage_tank_not_sensible
    assert_warning_if_arg_less_than(25, "storage_tank_volume",
                                    "A storage tank volume of less than 25 gallons and a certified rating is not commercially available. Please review the input.")
    assert_warning_if_arg_greater_than(100, "storage_tank_volume",
                                       "A hot water heater with a storage tank volume of greater than 100 gallons and a certified rating is not commercially available. Please review the input.")
  end
  
  def test_warning_if_rated_energy_factor_too_low
    assert_warning_if_arg_less_than(0.48, "rated_energy_factor",
                                    "Rated Energy Factor for Commercially available Gas Storage Water Heaters should be greater than 0.48")
  end
  
  def test_warning_if_rated_energy_factor_too_high
    assert_warning_if_arg_greater_than(0.82, "rated_energy_factor",
                                    "Rated Energy Factor for Commercially available Gas Storage Water Heaters should be less than 0.82")
  end
  
  def test_water_heater_capacity_should_be_in_sensible_range
    assert_warning_if_arg_less_than(25,"water_heater_capacity",
                                    "Commercially Available Gas Storage Water Heaters should have a minimum Nominal Capacity of 25 kBtu/h.")
    assert_warning_if_arg_greater_than(75,"water_heater_capacity",
                                       "Commercially Available Gas Storage Water Heaters should have a maximum Nominal Capacity of 75 kBtu/h.")
  end

  def test_water_heater_recovery_efficiency_should_be_in_sensible_range
    assert_warning_if_arg_less_than(0.70, "water_heater_recovery_efficiency",
                                    "Commercially Available Gas Storage Water heaters should have a minimum rated recovery efficiency of 0.70.")
    assert_warning_if_arg_greater_than(0.90, "water_heater_recovery_efficiency",
                                       "Commercially Available Gas Storage Water heaters should have a maximum rated recovery efficiency of 0.90.")
  end
  ...
  def test_warning_if_building_america_measure
    ...
    for protocol in ba_protocols do
      set_argument("use_case", "Create a water heater representing B10 Benchmark standard")
      @measure.run(@model, @runner, @argument_map)
      warnings = @runner.result.warnings.collect{|w| w.logMessage }
      assert_includes warnings, expected_warning, "Warning not generated for '#{protocol}'"
      
      # "General"" is not a BA protocol
      set_argument("use_case", "General")
      @measure.run(@model, @runner, @argument_map)
      warnings = @runner.result.warnings.collect{|w| w.logMessage }
  ...
  def test_final_conditions_include_name_capacity_and_size_of_new_heater
    ...
    assert_includes final_condition, "37 kBtu/h"
  ...
  ["NCTH", "B10 Benchmark"].each do |standard|
    [["1","1.5",30, 0.11356 ], ["3","2", 40,0.15142], ["2","1", 30,0.11356], ["4", "3",50, 0.18927 ],].each do | testcase |
      (beds, baths, gallons, si_result) = testcase
      define_method("test_#{standard}_with_#{beds}_and_#{baths}_has_#{gallons}_gallon_tank") do
        set_argument("use_case", "Create a water heater representing #{standard} standard")
        ...
        assert_in_delta si_result, heater.tankVolume.get, 0.00001
      end
    end
  end
  ...
  # This ia a doubly-parameterized test case, testing 4 table values for each of NCTH and B10 cases
  ["NCTH", "B10 Benchmark"].each do |standard|
    [["1","1.5",36 ], ["5","2", 48], ["2","1", 36], ["4", "3",38 ],].each do | testcase |
      ...
      define_method("test_#{standard}_with_#{beds}_and_#{baths}_has_#{capacity}_capacity") do
        ...
        refute_nil heater
        refute heater.tankVolume.empty?
        si_result = capacity * 1000 / 3.41214163513308 # (kBtu/hr to W conversion)
  ...
  def test_BA_Retrofit_has_capacity_from_input
    set_argument("water_heater_capacity", 35)
    set_argument("use_case", "Create a water heater representing BA Pre-Retrofit Case standard")
    @measure.run(@model, @runner, @argument_map)
  ...
  def test_water_heater_off_cycle_loss_coefficient_uses_36kBtu_per_hour_if_capacity_autosized
    set_argument("water_heater_recovery_efficiency", 0.75)
    set_argument("rated_energy_factor", 0.65)
    set_argument("water_heater_capacity", 0) # flag for autosize
    @measure.run(@model, @runner, @argument_map)
    
    heater = @model.getWaterHeaterMixeds[0]
    assert_in_delta 2.220, heater.offCycleLossCoefficienttoAmbientTemperature.get, 0.001
  end

  [["1","2", 0.75, 36, 3.329061881],
   ["5","3", 0.95, 64, 9.091033843]].each do |beds, baths, thermal_eff, cap, watts_per_kelvin|
    define_method("test_ba_protocol_water_heater_loss_coefficients_for_#{beds}_beds_#{baths}_baths_#{thermal_eff}_efficiency_and_#{cap}_capacity_is_#{watts_per_kelvin}") do
      set_argument("use_case", "Create a water heater representing NCTH standard")
      set_argument("number_of_bedrooms", beds)
      set_argument("number_of_bathrooms", baths)
      set_argument("water_heater_recovery_efficiency", thermal_eff)
      set_argument("water_heater_capacity", cap)
  ...
  def test_result_returns_info_messages
    ...
    assert_info_contains "Water heater '#{heater.name.get}' has a heater maximum capacity of 37 kBtu/h"
    assert_info_contains "Water heater '#{heater.name.get}' has a heater minimum capacity of 0 kBtu/h"
    assert_info_contains "Water heater '#{heater.name.get}' has a heater fuel type of 'NaturalGas'"
    assert_info_contains "Water heater '#{heater.name.get}' has a heater thermal efficiency of 0.85"
    assert_info_contains "Water heater '#{heater.name.get}' has an ambient temperature indicator of 'ThermalZone'"
    assert_info_contains "Water heater '#{heater.name.get}' has an on-cycle loss coefficient to ambient temperature of ", "W/K"
    assert_info_contains "Water heater '#{heater.name.get}' has an on-cycle loss fraction to thermal zone of 1.0"
    assert_info_contains "Water heater '#{heater.name.get}' has an off-cycle loss coefficient to ambient temperature of ", "W/K"
  ...
  def make_argument_map
    ...
    argMap = {
      ...
      "rated_energy_factor" => 0.61,
      "shw_setpoint_temperature" => 130,
      "water_heater_location" => "Thermal Zone 1",
      "water_heater_capacity" => 37,
      "water_heater_recovery_efficiency" => 0.85,
