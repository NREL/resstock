...
class ResidentialHotWaterHeaterTankGas < OpenStudio::Ruleset::ModelUserScript
    ...
    def name
        return "Set Residential Gas Tank Water Heater"
    end
    ...
    def run(model, runner, user_arguments)
        ...
        units.each do |unit|
            ...
            new_heater = Waterheater.create_new_heater(sch_unit_index, Constants.ObjectNameWaterHeater(unit.name.to_s), cap, Constants.FuelTypeGas, vol, nbeds, nbaths, ef, re, t_set, water_heater_tz, oncycle_p, offcycle_p, Constants.WaterHeaterTypeTank, 0, File.dirname(__FILE__), model, runner)
