...
class AddOSWaterHeaterMixedTanklessElectric_Test < Test::Unit::TestCase
  
  def setup
    # create an instance of the measure
    @measure = AddOSWaterHeaterMixedTanklessElectric.new
  ...
  def test_second_argument_is_rated_energy_factor
    ...
    assert_equal "Rated Energy Factor of Electric Tankless Water Heater. This field is ignored for NCTH and B10 protocols.", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_fourth_argument_is_water_heater_location
    ...
    assert_equal "Thermal Zone where the Electric Tankless Water Heater is located", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_fifth_argument_is_water_heater_capacity
    ...
    assert_equal "The nominal capacity [kW] of the electric storage water heater. Set to 0 to have this field autosized.", arg.displayName
    refute arg.hasDefaultValue
  ...
  def test_water_heater_capacity_must_be_greater_than_0kW
    assert_error_if_arg_less_than(0, "water_heater_capacity", "Electric Tankless Water Heater Nominal Capacity must be greater than 0 kW.")
  end
  ...
  def test_water_heater_capacity_of_negative_one_is_an_error
    ...
    errors = @runner.result.errors.collect{ |w| w.logMessage}
    assert_includes errors, "Electric Tankless Water Heater Nominal Capacity must be greater than 0 kW."    
  end
  ...
  def test_warning_if_rated_energy_factor_too_low
    assert_warning_if_arg_less_than(0.98, "rated_energy_factor",
                                    "AHRI Certified Energy Factors for Commercially available Electric Tankless Water Heaters should be 0.98 or greater.")
  end
  
  def test_water_heater_capacity_should_be_in_sensible_range
    assert_warning_if_arg_less_than(2.4,"water_heater_capacity",
                                    "Commercially Available Electric Tankless Water Heaters should have a minimum Nominal Capacity of 2.4 kW.")
  end
  ...
  def test_initial_conditions_include_existing_water_heaters
    ...
    new_tankless.setName("Test Existing Tankless Heater Electric")
    new_tankless.setHeaterMaximumCapacity(2578)
    new_tankless.setTankVolume(OpenStudio::Quantity.new(1,gal))
    new_tankless.setHeaterFuelType("Electricity")
    ...
    assert_includes initial_condition, "2014 W", "Initial condition should include existing heater's capacity"
    ...
    assert_includes initial_condition, "1964 W", "Initial condition should include existing heater's capacity"
    assert_includes initial_condition, "47 gal", "Initial condition should include existing heater's size"   

    assert_includes initial_condition, "Test Existing Tankless Heater Electric"
    assert_includes initial_condition, "Test Plant Loop 2", "Initial condition should include existing heater's plant loop"
    assert_includes initial_condition, "2578 W", "Initial condition should include existing heater's capacity"
  ...
  def test_general_has_capacity_from_input
    set_argument("water_heater_capacity", 2.7)
    @measure.run(@model, @runner, @argument_map)
