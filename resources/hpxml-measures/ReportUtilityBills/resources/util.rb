...
def process_usurdb(filepath, export_zip: true)
  ...
  # JH: revised to process any csv generated by nrgx.openei using openei
  require 'csv'
  require 'json'
  require 'zip'
  ...
  residential_rates.each do |residential_rate|
    ...
    json_files << "#{filename}.json"
  end

  if !export_zip
    puts "Created in #{rates_dir}: #{json_files}"
