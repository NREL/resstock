...
class MiscLoads
  def self.apply_plug(model, plug_load, obj_name, cfa, living_space, schedules_file)
    ...
    if not plug_load.schedule.nil?
      space_design_level = schedules_file.calc_design_level_from_annual_kwh(col_name: plug_load.schedule, annual_kwh: kwh)
      sch = schedules_file.create_schedule_file(col_name: plug_load.schedule)
    else
