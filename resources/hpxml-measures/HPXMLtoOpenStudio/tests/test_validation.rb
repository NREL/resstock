...
class HPXMLtoOpenStudioValidationTest < Minitest::Test
  ...
  def test_schema_schematron_error_messages
    ...
    all_expected_errors = { 'boiler-invalid-afue' => ['Expected AnnualHeatingEfficiency[Units="AFUE"]/Value to be less than or equal to 1'],
                            ...
                            'enclosure-conditioned-missing-ceiling-roof' => ['There must be at least one ceiling/roof adjacent to conditioned space. [context: /HPXML/Building/BuildingDetails/Enclosure[*/*[InteriorAdjacentTo="conditioned space"]]]',
                            ...
                            'enclosure-conditioned-missing-exterior-wall' => ['There must be at least one exterior wall adjacent to conditioned space. [context: /HPXML/Building/BuildingDetails/Enclosure[*/*[InteriorAdjacentTo="conditioned space"]]]'],
                            'enclosure-conditioned-missing-floor-slab' => ['There must be at least one floor/slab adjacent to conditioned space. [context: /HPXML/Building/BuildingDetails/Enclosure[*/*[InteriorAdjacentTo="conditioned space"]]]'],
                            ...
                            'invalid-soil-type' => ["Expected SoilType to be 'sand' or 'silt' or 'clay' or 'loam' or 'gravel' or 'unknown' [context: /HPXML/Building/BuildingDetails/BuildingSummary/Site/Soil]"],
    ...
    all_expected_errors.each_with_index do |(error_case, expected_errors), i|
      ...
      if ['boiler-invalid-afue'].include? error_case
        ...
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base.xml'))
        hpxml.site.ground_diffusivity = 0.0
        ...
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base.xml'))
        hpxml.hvac_distributions[0].ducts[1].duct_location = HPXML::LocationManufacturedHomeBelly
      elsif ['manufactured-home-reference-water-heater'].include? error_case
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base.xml'))
        hpxml.water_heating_systems[0].location = HPXML::LocationManufacturedHomeBelly
      elsif ['manufactured-home-reference-floor'].include? error_case
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base-foundation-vented-crawlspace.xml'))
  ...
  def test_ruby_error_messages
    ...
    all_expected_errors.each_with_index do |(error_case, expected_errors), i|
      ...
      if ['battery-bad-values-max-not-one'].include? error_case
        ...
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base-hvac-air-to-air-heat-pump-1-speed.xml'))
        hpxml.heat_pumps[0].compressor_lockout_temp = 35.0
        hpxml.heat_pumps[0].backup_heating_lockout_temp = 35.0
        ...
        hpxml = HPXML.new(hpxml_path: File.join(@sample_files_path, 'base-hvac-geothermal-loop.xml'))
        hpxml.heat_pumps[0].geothermal_loop_idref = nil
