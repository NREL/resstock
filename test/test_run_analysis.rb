...
class TestRunAnalysis < Minitest::Test
  ...
  def _verify_outputs(cli_output_log, testing = false)
    ...
    File.readlines(cli_output_log).each do |message|
      ...
      next if _expected_warning_message(message, 'Either a primary heating system was not found, or it was found but is a shared system; not setting it as heat pump backup.')
